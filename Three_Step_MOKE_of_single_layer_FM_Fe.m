%% Step 1, 2 & 3-- MOKE response as a fucntion of FM thickness (Fe)

clear all
% Define all the parameters and constant 
%Parameters for step one
tau_T = 0.41e-6; %torque at top surface (conventional) (J/(m^2)) -1.8e-6;
...
M_s = 2.24/mu; %saturation magnetization (A/m) (from mu*Ms = 1.06T)
H_a = 79.77*27074/2/(a/1e-9); %surface anisotropy effective field (A/m) 
J_ex = 2*19.7e-12/a; %interlayer exchange strength (J/m^2) %0.64 is from the scaling of thickness factor 0.8

%Parameters for step three
lambda = 780e-9;  % wavelength (m)
refrac_layer = 2.69+1i*3.68; %refractive index of the FM layer(for Py,2.2+1i*4.2) (14.78nm penetration depth)
...
Q_FM = 0.03-1i*0.01; % magneto-optic coefficient of Co (0.043+1i*0.007)
...
SOT = -40;%Kerr rotation 934(16Py) 1310, 740
SOT2 = 2; %Kerr Ellipticity -236, -201
%err = [7*1.62, 10*1.24, 6*1.11, 23*1.05, 2*9*1.00, 2*8*0.99, 2*26*0.96, 2*12*0.94]/5; %error bar
