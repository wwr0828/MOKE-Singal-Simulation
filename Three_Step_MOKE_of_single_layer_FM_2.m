...
tau_T = -1.8e-6; %Spin Hall angle for x-spin spin current (conventional)
tau_B = -tau_T; %Spin Hall angle for z-spin spin current (spin rotation)
...
%data2 = [tt/1e-9; m2']; %/max(m1)
%fileID2 = fopen('Magnetization distribution_wo_Ha_5layer.txt','w');
%fprintf(fileID2, '%f %e\n',data2);
%fclose(fileID2);

figure(1)
plot(tt/1e-9,m1,'r-','LineWidth', 4); %spin current distribution (two curves)
%hold on
%plot(tt/1e-9,m2,'b','LineWidth', 4);
%set(gca,'FontSize',20)
...
% Define all the parameters and constant 
%Parameters for step one
tau_T = -1.8e-6; %torque at top surface (conventional) (J/(m^2)) -1.8e-6;
tau_B = -1.01*tau_T; %torque at bottom surface 
J_e = 1.25*0.8e+11; %charge current density  A/m^2)
%sigma = 9.43e+6; %electrical conductivity (S/m) Pt
%N_s = 1.6e+47; % electron density of states (/J/m^3)
%l_sf = 1000e-9; %l_sf; spin-flip length, large! 1000nm (m)
...
%%
TT = a:a:T_max;
Thickness = 0.8*[5, 10, 15, 20, 30, 40, 60]; %experiemental thickness points`
...
SOT = [17, 323, 856, 1050, 1102, 1184, 1298]/10;%Kerr rotation 934(16Py) 1310, 740
SOT2 = [-113, -55, -112, -152, -249 ,-299 ,-308]; %Kerr Ellipticity -236, -201
%err = [7*1.62, 10*1.24, 6*1.11, 23*1.05, 2*9*1.00, 2*8*0.99, 2*26*0.96, 2*12*0.94]/5; %error bar

data = [TT/1e-9; real(Phi_S')*1e9];
fileID = fopen('thickness dependence rotation_2.txt','w');
fprintf(fileID, '%f %e\n',data);
fclose(fileID);

data2 = [TT/1e-9; imag(Phi_S')*1e9];
fileID2 = fopen('thickness dependence ellipticity_2.txt','w');
fprintf(fileID2, '%f %e\n',data2);
fclose(fileID2);
